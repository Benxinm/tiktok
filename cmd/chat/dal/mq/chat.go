...
func (c *ChatMQ) DealWithMessageToUser(msg <-chan amqp.Delivery) {
	for req := range msg {
		...
		err = convert(message, middle_message)
		if err != nil {
			klog.Error(err)
			continue
		}
		...
		err = cache.CreateMessage(context.TODO(), key, revkey, message.CreatedAt.UnixMilli(), string(msg))
		if err != nil {
			klog.Error(err)
			Mu.Unlock()
			continue
...
func convert(message *cache.Message, tempMessage *MiddleMessage) (err error) {
